...
type fakeAnalyzer struct {
	name            string
	analyzeFunc     func(interaction.Interface) (analyzer.Result, error)
	callCount       int
...
func (f *fakeAnalyzer) Analyze(inter interaction.Interface) (analyzer.Result, error) {
	f.callCount++
	f.lastInteraction = inter
	return f.analyzeFunc(inter)
}

func (f *fakeAnalyzer) withResult(result analyzer.Result) *fakeAnalyzer {
	f.analyzeFunc = func(interaction.Interface) (analyzer.Result, error) {
		return result, nil
	}
	return f
}
